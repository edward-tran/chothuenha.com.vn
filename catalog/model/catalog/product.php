...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, p.price, m.name AS manufacturer, k.name AS post_type,
			...
			(SELECT r2.full_name FROM " . DB_PREFIX . "customer r2 WHERE r2.customer_id = p.customer_id) AS customer_full_name,
			(SELECT r2.image FROM " . DB_PREFIX . "customer r2 WHERE r2.customer_id = p.customer_id) AS customer_image,
			...
			(SELECT name FROM " . DB_PREFIX . "direction_house r7 WHERE r7.direction_house_id = p.direction_balcony_id) AS balcony,
		...
		if ($query->num_rows) {
			return array(
				...
				'direction_balcony_id'       => $query->row['direction_balcony_id'],
				'balcony'       => $query->row['balcony'],
				'type_name'       => $query->row['type_name'],
				'customer_full_name'       => $query->row['customer_full_name'],
				...
				'facade'           => $query->row['facade'],
				'large_road'           => $query->row['large_road'],
				// 'depth'           => $query->row['depth'],
				// 'direction'           => $query->row['direction'],
				// 'project_size'           => $query->row['project_size'],
			);
		} else {
			return false;
		}
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['type_id'])) {
			$sql .= " AND p.type_id = '" . (int)$data['type_id'] . "'";
		}

		if (!empty($data['bedroom'])) {
			$sql .= " AND p.bedroom = '" . (int)$data['bedroom'] . "'";
		}
		...
		if (!empty($data['direction_house_id'])) {
			$sql .= " AND p.direction_house_id = '" . (int)$data['direction_house_id'] . "'";
		}
		...
		if (!empty($data['acreage_to'])) {
			$acreage_from 	= (int)$data['acreage_from'];
			$acreage_to		= (int)$data['acreage_to'];
			$sql 			.= " AND (p.acreage BETWEEN $acreage_from AND $acreage_to)";
		}
	...
	public function getProductByTypeId($type_id, $product_id) {
	...
	public function getCategoriesByProductId($product_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "product_to_category a LEFT JOIN " . DB_PREFIX . "category_description cd ON (a.category_id = cd.category_id) WHERE a.product_id = '" . (int)$product_id . "'");
	...
	public function getProductVieweds() {
        ...
        if(isset($this->session->data['product_viewed'])) {
            ...
            foreach ($this->session->data['product_viewed'] as $key => $quantity) {
               ...
               $data[] = array(
                    ...
                    'post_type_name'   => $query->row['post_type_name'],
                    'type_name'   => $query->row['type_name'],
                    'number_image'   => $query->row['number_image'],
                    'acreage'   => $query->row['acreage'],
                    'bedroom'   => $query->row['bedroom'],
                    'bathroom'   => $query->row['bathroom'],
                    'wishlist'   => $query->row['wishlist'],
