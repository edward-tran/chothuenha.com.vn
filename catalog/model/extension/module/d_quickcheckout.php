...
public function array_merge_r_d() {
    ...
    */
    public function getUpdateInfo($mbooth_xml, $status = 1){
        ...
        $request = 'http://iiwi..com/api/1/index.php?routing=extension/check&mbooth=' . $mbooth_xml . '&store_url=' . $customer_url . '&module_version=' . $current_version . '&language_code=' . $language_code . '&iiwi_version=' . VERSION . '&ip='.$ip . '&status=' .$status;
    ...
    public function download_extension($mbooth_xml, $filename  = false ) {
        ...
        curl_setopt($ch, CURLOPT_URL, 'http://iiwi..com/api/1/extension/download/?mbooth=' . $mbooth_xml.'&iiwi_version='.VERSION);
