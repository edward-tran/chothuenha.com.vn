...
class ModelExtensionDShopunityMbooth extends Model {
    ...
    public function downloadExtension($codename){
        ...
        if($mbooth){
            ...
            //add install.xml file for iiwi automatic installer.
    ...
    //             $routing = array_shift($parts);
    //         }
    //     }

    //     if(isset($routing) && isset($parts)){

    //         try{
    //             if(VERSION < '2.3.0.0'){
    //                 $content = file_get_contents(str_replace('&amp;', '&', $this->url->link($routing, implode('&', $parts).'&token='.$this->session->data['token'], 'SSL')));
    //             }else{
    //                 parse_str(implode("&", $parts), $vars);
    //                 $this->request->get['extension'] = $vars['extension'];
    //                 $this->load->controller($routing);
    //             }
