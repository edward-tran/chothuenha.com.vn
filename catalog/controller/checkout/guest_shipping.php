...
class ControllerCheckoutGuestShipping extends Controller {
	public function index() {
		...
		if (isset($this->session->data['shipping_address']['ward'])) {
			$data['ward'] = $this->session->data['shipping_address']['ward'];
		} else {
			$data['ward'] = '';
		}
	...
	public function save() {
		...
		if (!$json) {
			...
			if ((utf8_strlen(trim($this->request->post['ward'])) < 2) || (utf8_strlen(trim($this->request->post['ward'])) > 128)) {
				$json['error']['ward'] = $this->language->get('error_ward');
			}
		...
		if (!$json) {
			...
			$this->session->data['shipping_address']['ward'] = $this->request->post['ward'];
			...
			$ward_infos = $this->model_localisation_ward->getWardCode($this->request->post['city']);
			if ($ward_infos) {
				$this->session->data['shipping_address']['ward'] = $ward_infos['name'];
				$this->session->data['shipping_address']['ward_code'] = $ward_infos['ward_id'];
			} else {
				$this->session->data['shipping_address']['ward'] = '';
				$this->session->data['shipping_address']['ward_code'] = '';
			}
			//end custom
