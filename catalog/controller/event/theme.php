...
class ControllerEventTheme extends Controller {
	public function index(&$routing, &$args, &$template) {
	...
	public function override(&$routing, &$args, &$template) {
		...
		$theme_info = $this->model_design_theme->getTheme($routing, $theme);
		
		if ($theme_info) {
			$template = html_entity_decode($theme_info['code'], ENT_QUOTES, 'UTF-8');
		} elseif (is_file(DIR_TEMPLATE . $theme . '/template/' . $routing . '.twig')) {
			$this->config->set('template_directory', $theme . '/template/');
		} elseif (is_file(DIR_TEMPLATE . 'default/template/' . $routing . '.twig')) {
