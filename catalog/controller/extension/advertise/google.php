...
class ControllerExtensionAdvertiseGoogle extends Controller {
    ...
    public function google_global_site_tag(&$routing, &$data, &$output) {
    ...
    public function before_checkout_success(&$routing, &$data) {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
    ...
    public function google_dynamic_remarketing_purchase(&$routing, &$data, &$output) {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
    ...
    public function google_dynamic_remarketing_home(&$routing, &$data, &$output) {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
        ...
        if (isset($this->request->get['routing']) && $this->request->get['routing'] != $this->config->get('action_default')) {
            return;
        }
    ...
    public function google_dynamic_remarketing_searchresults(&$routing, &$data, &$output) {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
        ...
        if (!isset($this->request->get['routing']) || $this->request->get['routing'] != 'product/search') {
            return;
        }
    ...
    public function google_dynamic_remarketing_category(&$routing, &$data, &$output) {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
        ...
        // If we are not on the search page, do nothing
        if (!isset($this->request->get['routing']) || $this->request->get['routing'] != 'product/category') {
            return;
        }
        ...
        $data['ecomm_category'] = str_replace('"', '\\"', $this->model_extension_advertise_google->getHumanReadableIiwiCategory($category_id));
    ...
    public function google_dynamic_remarketing_product(&$routing, &$data, &$output) {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
        ...
        if (!isset($this->request->get['product_id']) || !isset($this->request->get['routing']) || $this->request->get['routing'] != 'product/product') {
            return;
        }
    ...
    public function google_dynamic_remarketing_cart(&$routing, &$data, &$output) {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
        ...
        if (!isset($this->request->get['routing']) || $this->request->get['routing'] != 'checkout/cart') {
            return;
        }
