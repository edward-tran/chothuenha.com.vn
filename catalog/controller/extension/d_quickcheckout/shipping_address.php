...
class ControllerExtensionDQuickcheckoutShippingAddress extends Controller {
	...
	public function index($config){
        ...
        return $this->model_extension_d_iiwi_patch_load->view($template, $data);
	}
    ...
    public function prepare($json){
        ...
        if(isset($this->request->post['shipping_address'])){
            ...
            if(isset($this->session->data['shipping_address']['ward_id']) && isset($this->request->post['shipping_address']['ward_id'])){
                if($this->session->data['shipping_address']['ward_id'] != $this->request->post['shipping_address']['ward_id']){
                    $json['payment_address_refresh'] = true;
                    $json['shipping_address_refresh'] = true;
                }
            }
