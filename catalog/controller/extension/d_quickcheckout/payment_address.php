...
class ControllerExtensionDQuickcheckoutPaymentAddress extends Controller {
    ...
    public function index($config){
        ...
        return $this->model_extension_d_iiwi_patch_load->view('d_quickcheckout/payment_address', $data);
	}
    ...
    public function prepare($json){
        ...
        if(isset($this->request->post['payment_address'])){
            ...
            if(isset($this->session->data['payment_address']['ward_id']) && isset($this->request->post['payment_address']['ward_id'])){
                if($this->session->data['payment_address']['ward_id'] != $this->request->post['payment_address']['ward_id']){
                    $json['payment_address_refresh'] = true;
                    $json['shipping_address_refresh'] = true;
                }
            }
        ...
        if($this->customer->isLogged()){
            if(empty($this->session->data['payment_address']['address_id'])){
                if($this->model_account_address->getAddresses()){
                    ...
                    $this->session->data['payment_address']['zone_id'] = 1;
                }
            }
