...
class ControllerExtensionPaymentKlarnaCheckout extends Controller {
	...
	public function validation() {
		...
		if ($validate) {
			...
			if ($order_info) {
				...
				if (round($this->currency->format($total, $order_info['currency_code'], $order_info['currency_value'], false) * 100) != $request->order_amount) {
					$this->model_extension_payment_klarna_checkout->log('Klarna Checkout order_amount does not match session order total. Klarna Request: ' . $request->order_amount . '. Iiwi: ' . round($this->currency->format($total, $order_info['currency_code'], $order_info['currency_value'], false) * 100));
		...
		if ($validate) {
			...
			// Update Iiwi order with payment and shipping details
			...
			//If region is passed, try to update Iiwi order with correct region/zone
