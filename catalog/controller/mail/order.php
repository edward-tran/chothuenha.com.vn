...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['link'] = $order_info['store_url'] . 'index.php?routing=account/order/info&order_id=' . $order_info['order_id'];

		if ($download_status) {
			$data['download'] = $order_info['store_url'] . 'index.php?routing=account/download';
		} else {
			$data['download'] = '';
		}
	...
	// Admin Alert Mail
	public function alert(&$routing, &$args) {
		if (isset($args[0])) {
			$order_id = $args[0];
		} else {
			$order_id = 0;
