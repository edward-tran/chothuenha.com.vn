...
class ControllerInformationContact extends Controller {
	...
	public function loadFormPoupContact() {
		$this->response->setOutput($this->load->view('information/load_form_poup_contact'));
	}

	public function index() {
		$this->load->language('information/contact');

		$this->document->addStyle('catalog/view/theme/beta/stylesheet/contact.css?'.rand());
	...
	protected function validate() {
		...
		if ((utf8_strlen($this->request->post['telephone']) < 10) || (utf8_strlen($this->request->post['telephone']) > 3000)) {
	...
	public function send() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$data['email'] = $this->request->post['email'];
			$data['telephone'] = $this->request->post['telephone'];
			$data['message'] = $this->request->post['enquiry'];
			$data['title'] = html_entity_decode(sprintf($this->language->get('email_subject'), $this->request->post['name']), ENT_QUOTES, 'UTF-8');
			$mail->setHtml($this->load->view('information/form_mail_contact', $data));
			$mail->send();
