...
qc.ShippingAddress = qc.Model.extend({
    ...
    changeAddress: function(address_id){
        ...
        var that = this;
        $.post('index.php?routing=extension/d_quickcheckout/shipping_address/update', { 'shipping_address' :  json.shipping_address }, function(data) {
    ...
    validate: function(attrs, options){
        var errors = [];
        if (!_.isUndefined(this.get('shipping_address.'+options.key)) && !_.isNull(this.get('shipping_address.'+options.key))
        && this.get('shipping_address.'+options.key).length > 0 && !_.isUndefined(attrs.shipping_address[options.key])
        && attrs.shipping_address[options.key].length == 0) {
