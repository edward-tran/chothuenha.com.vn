...
var that=this;$.post('index.php?routing=extension/d_quickcheckout/payment_address/update',
	{'payment_address':json.payment_address},function(data){that.updateForm(data);},'json').error();},
...
updateField:function(name,value){
	...
	qc.payment_address_waiting=setTimeout(function(){
		$.post('index.php?routing=extension/d_quickcheckout/payment_address/update',
...
emailVal=email.val();var that=this;$.ajax({url:"index.php?routing=extension/d_quickcheckout/field/validate_email",async:false,method:'GET',dataType:"json",data:'email='+emailVal,success:function(data){if(data!=true){valid=false
...
function getDistrictCode() {
	qc.payment_address_waiting=setTimeout(function(){
			var zone_id = $('#payment_address_zone_id').val();
			$.ajax({
			url: 'index.php?routing=localisation/district/getDistrictCode&province_id=' + zone_id,
			dataType: 'json',			
			success: function(json) {
				html = '<option value=""> --- '+text_district+' --- </option>';
				if (json['district'] != '') {
					for (i = 0; i < json['district'].length; i++) {
				html += '<option value="' + json['district'][i]['zone_district_id'] + '"';
						if (json['district'][i]['name'] == '') {
				...
				$('select[id=\'payment_address_city\']').html(html);
			},
...
function getWardCode() {
	qc.payment_address_waiting=setTimeout(function(){
			var district_id = $('#payment_address_city').val();
			$.ajax({
			url: 'index.php?routing=localisation/ward/getWardCode&district_id=' + district_id,
			dataType: 'json',			
			success: function(json) {
				html = '<option value=""> --- '+text_district+' --- </option>';
				if (json['ward'] != '') {
				...
				$('select[id=\'payment_address_ward\']').html(html);
			},
			error: function(xhr, ajaxOptions, thrownError) {
				console.log(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
			}
