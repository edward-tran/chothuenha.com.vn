...
(function (window, document, $, undefined) {
	...
	$.extend(F, {
		...
		jumpto: function ( index, direction, routingr ) {
			var current = F.current;
		...
		_start: function (index) {
			...
			if (!type) {
				...
				if (F.current && F.routingr && F.routingr !== 'jumpto') {
					F.current.index = index;

					return F[ F.routingr ]( F.direction );
				}
		...
		_preloadImages: function() {
			...
			for (i = 1; i <= cnt; i += 1) {
				...
				if (item.type === 'image' && item.data_target) {
					new Image().src3 = item.data_target;
				}
			}
		},
