<?php
// Heading
$_['heading_title']            = 'Giỏ Hàng';

// Text
$_['text_success']           = 'Thành công: Bạn đã thêm <a href="%s">%s</a> vào <a href="%s">giỏ hàng</a>!';
$_['text_remove']            = 'Thành công: Bạn đã sửa đổi giỏ hàng của bạn!';
$_['text_login']             = 'Lưu ý: Bạn phải <a href="%s">đăng nhập</a> hoặc <a href="%s">đăng kí thành viên</a> để xem giá!';
$_['text_items']       = '%s';
$_['text_points']              = 'Điểm thưởng: %s';
$_['text_next']                = 'Tiếp theo?';
$_['text_next_choice']         = 'Chọn nếu bạn có mã giảm giá hoặc điểm thưởng mà bạn muốn sử dụng.';
$_['text_empty']               = 'Giỏ hàng của bạn đang trống!';
$_['text_day']                 = 'Ngày';
$_['text_week']                = 'Tuần';
$_['text_semi_month']          = 'Nửa tháng';
$_['text_month']               = 'Tháng';
$_['text_year']                = 'Năm';
$_['text_trial']               = '%s every %s %s for %s payments then ';
$_['text_recurring']           = '%s every %s %s';
$_['text_payment_cancel']      = 'Cho đến khi hủy';
$_['text_recurring_item']      = 'Mục Định kỳ';
$_['text_payment_recurring']   = 'Hồ sơ thanh toán';
$_['text_trial_description']   = '%s every %d %s(s) for %d payment(s) then';
$_['text_payment_description'] = '%s every %d %s(s) for %d payment(s)';
$_['text_payment_cancel']      = '%s every %d %s(s) until canceled';

// Column
$_['column_image']           = 'Hình ảnh';
$_['column_name']            = 'Tên dự án';
$_['column_model']           = 'Mã hàng';
$_['column_quantity']        = 'Số lượng';
$_['column_price']           = 'Đơn Giá';
$_['column_total']           = 'Tổng cộng';;

// Error
$_['error_stock']            = 'Dự án đánh dấu *** không có đủ số lượng trong kho!';
$_['error_minimum']          = 'Số lượng đặt tối thiểu cho %s là %s!';	
$_['error_required']         = '%s không được bỏ trống!';	
$_['error_product']          = 'Cảnh báo: Không có dự án trong giỏ hàng của bạn!';	
$_['error_recurring_required'] = 'Vui lòng chọn một khoản thanh toán định kỳ!';